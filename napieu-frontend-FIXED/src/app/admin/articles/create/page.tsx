...
export default function CreateArticlePage() {
  ...
  const [imageWidth, setImageWidth] = useState(50); // Percentage width (20-100)
  ...
  const loadDraft = () => {
    try {
      ...
      if (draft) {
        ...
        const hasContent = parsed.title || parsed.content;
        
        if (hasContent && confirm('📝 Found a saved draft. Continue editing?')) {
          setFormData(parsed);
          setLastSaved(new Date());
        }
      }
  ...
  const insertImage = () => {
    ...
    // Convert percentage to inline style
  ...
  const handleSubmit = async (e: React.FormEvent, saveAsDraft = false) => {
    ...
    const submitData = { ...formData, published: !saveAsDraft };
    ...
    try {
      ...
      if (response.ok) {
        alert(saveAsDraft ? '💾 Draft saved!' : '✅ Article published!');
        clearDraft();
    ...
    <div>
      ...
      <div className="mb-8 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button
            onClick={() => {
              if (confirm('⚠️ Discard changes and go back?')) {
                clearDraft();
                router.push('/admin/articles');
              }
            }}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-4">
                ...
                >
                  <Save size={20} />
                  {loading ? '⏳ Saving...' : formData.published ? '🚀 Publish Article' : '💾 Save as Draft'}
                </button>
                ...
                {formData.published && (
                  <button
                    ...
                    disabled={loading}
                    className="w-full bg-gray-600 text-white px-6 py-2 rounded-lg hover:bg-gray-700 transition-colors disabled:opacity-50 text-sm font-medium"
                  >
